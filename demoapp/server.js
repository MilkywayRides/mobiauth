...
function encryptEnvelope(payload) {
  ...
  const unsigned = { timestamp, nonce, iv: iv.toString("base64"), tag: tag.toString("base64"), ciphertext: ciphertext.toString("base64") };
...
function decryptEnvelope(envelope) {
  ...
  const decipher = crypto.createDecipheriv("aes-256-gcm", ENC_KEY, Buffer.from(envelope.iv, "base64"));
...
async function controlAction(actionPayload) {
  ...
  const res = await fetch(`${AUTH_SERVICE_URL}/api/secure/control`, {
    method: "POST",
    headers: { "Content-Type": "application/json", "x-control-token": CONTROL_SERVICE_TOKEN },
    body: JSON.stringify(envelope),
  });
  const body = await res.json();
  if (!res.ok) throw new Error(body.error || `control request failed (${res.status})`);
  return decryptEnvelope(body);
}

const page = (title, body) => `<!DOCTYPE html><html><head><meta charset="utf-8" /><title>${title}</title><style>
body{font-family:Inter,Arial,sans-serif;max-width:1200px;margin:20px auto;padding:0 16px;background:#0b1020;color:#e5e7eb}
...
button{cursor:pointer;background:#2563eb;border:0} pre{white-space:pre-wrap;word-break:break-word;background:#020617;padding:12px;border-radius:8px}
.grid{display:grid;grid-template-columns:1fr 1fr;gap:16px}
</style></head><body><h1>MobiAuth Demo App (External Controller)</h1><p>Advanced OAuth setup + API key + users from external app.</p>${body}</body></html>`;
  ...
  try {
    if (req.method === "GET" && req.url === "/") {
      ...
      <div class="grid">
        ...
        <div class="card">
          <h3>Create OAuth client (advanced)</h3>
          <form method="post" action="/create-oauth-client">
            <label>User ID</label><input name="userId" required />
            <label>Client Name</label><input name="name" required />
            <label>Description</label><input name="description" />
            <label>Website (https)</label><input name="website" placeholder="https://example.com" />
            <label>Logo URL</label><input name="logo" placeholder="https://example.com/logo.png" />
            <label>Redirect URIs (one per line)</label><textarea name="redirectUris" rows="4" required></textarea>
            <label>Scopes (comma separated)</label><input name="scopes" value="profile,email,read:user" />
      ...
      <div class="card">
        ...
        <form method="post" action="/set-role">
          <label>User ID</label><input name="userId" required />
          <label>Role</label><select name="role"><option>user</option><option>admin</option><option>super_admin</option></select>
    ...
    if (req.method === "POST" && req.url === "/set-role") {
      const body = await parseBody(req);
      await controlAction({ action: "set_user_role", userId: body.userId, role: body.role });
      return redirect(res);
    }

    if (req.method === "POST" && req.url === "/create-api-key") {
      const body = await parseBody(req);
      const scopes = String(body.scopes || "").split(",").map((s) => s.trim()).filter(Boolean);
      await controlAction({
    ...
    if (req.method === "POST" && req.url === "/create-oauth-client") {
      const body = await parseBody(req);
      const scopes = String(body.scopes || "").split(",").map((s) => s.trim()).filter(Boolean);
      const redirectUris = String(body.redirectUris || "").split("\n").map((s) => s.trim()).filter(Boolean);
      await controlAction({
